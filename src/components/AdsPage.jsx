...
function AdsItem({ ad }) {
  return (
    <Link to={"/ads/" + ad.id} className="bg-white mr-2 w-1/5 mb-4 rounded">
  ...
  return (
    <div>
      ...
      <div className="flex">
        <button
          onClick={openModal}
          className="flex items-center justify-center flex-col  bg-white mr-2 w-1/5 mb-4 p-3 border-gray-500 border-solid border-2"
        >
          <div className="text-3xl">+</div>
          <div>Create New Ad</div>
        </button>
        {ads
          .filter((ad) => ad.date > getDate())
          .map((ad) => (
            <AdsItem ad={ad} />
          ))}
      </div>
